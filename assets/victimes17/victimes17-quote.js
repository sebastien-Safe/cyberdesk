...
async function _quoteComputeO4() {
  ...
  try {
    ...
    _quoteO4Result = {
      distance_km: result.distance_km, forfait_eur: result.forfait_eur,
      coefficient_eur_km: result.coefficient_eur_km, amount_ht: result.amount_ht, city: result.city,
    };
    // Barème (forfait + coefficient) propre à l'agent connecté, réglé
    // dans Paramétrage → Profil → Facturation — renvoyé par l'Edge
    // Function (cyberdesk_user_settings), jamais lu depuis tarifs-
    // cyberdesk.json ni un réglage global.
    resultEl.textContent = `📍 ${result.city} : `
      + `forfait ${formatMoney(result.forfait_eur)} + ${result.distance_km} km (aller-retour) × ${formatMoney(result.coefficient_eur_km)}/km `
      + `= ${formatMoney(result.amount_ht)} HT — barème de votre profil (Paramétrage)`;
  ...
  _quoteTarifs.options.forEach(o => {
    ...
    if (o.type === 'fixe') {
      ...
      if (_quoteO4Result) {
        ...
        lines.push({
          label: `${o.label} — ${_quoteO4Result.distance_km} km aller-retour (${_quoteO4Result.city})`,
